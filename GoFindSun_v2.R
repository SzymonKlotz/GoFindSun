...
# UI aplikacji
ui <- fluidPage(
  theme = shinytheme("flatly"),  # Motyw Flatly
  ...
  observeEvent(input$find_weather, {
    ...
    if (nrow(filtered_data) == 0) {
      ...
      avg_scores <- scored_data %>%
        group_by(city) %>%
        summarise(
          mean_score = mean(total_score, na.rm = TRUE) * 100,  # Wynik w %
      ...
      output$no_result <- renderText("10 najlepszych miejsc na mapie zostało wyświetlonych.")
      output$map <- renderLeaflet({
        leaflet(data = top_matches) %>%
          addTiles() %>%
          addMarkers(
            lng = ~as.numeric(lon),
            lat = ~as.numeric(lat),
      ...
      output$score_table <- renderDT({
        avg_scores %>%
          ...
          select(Lp, City = city, Matching_percent = mean_score, Distance_km = distance_km, Airbnb_Link) %>%
          datatable(
            escape = FALSE,
            options = list(pageLength = 10, autoWidth = TRUE, width = "100%")  # Dodano dynamiczną szerokość
          )
      })
    }
  })
