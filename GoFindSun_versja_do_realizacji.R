install.packages(c("httr", "jsonlite", "dplyr", "tidyr", "leaflet", "lubridate", "shiny", "DT", "shinythemes"))
library(shiny)
library(httr)
library(jsonlite)
library(leaflet)
    ...
    batch_data <- lapply(seq_len(nrow(batch)), function(j) {
      ...
      tryCatch({
        ...
        data_frame <- data.frame(
          ...
          rain = ifelse(any(!is.null(data$list$rain.3h) & !is.na(data$list$rain.3h) & data$list$rain.3h > 0), "Yes", "No"),
          snow = ifelse(any(!is.null(data$list$snow.3h) & !is.na(data$list$snow.3h) & data$list$snow.3h > 0), "Yes", "No")
        )
        return(data_frame)
      }, error = function(e) {
  ...
  data %>%
    mutate(
      ...
      clouds_mediana_score = case_when(
        clouds_mediana < 20 ~ 1,
        clouds_mediana >= 20 & clouds_mediana <= 50 ~ 1,
        TRUE ~ -0.2
      ),
      ...
      humidity_mediana_score = case_when(
        ...
        humidity_mediana >= 0 & humidity_mediana < 40 ~ 1,
        humidity_mediana > 60 & humidity_mediana <= 70 ~ 0.7,
        TRUE ~ -0.2
      ),
      total_score = 0.2 * temp_mediana_score +
        0.2 * wind_score +
        0.2 * clouds_mediana_score +
        0.35 * (0.9 * rain_score + 0.1 * snow_score) +
...
calculate_cultural_score <- function(data) {
  data %>%
    mutate(
      ...
      humidity_mediana_score = case_when(
        humidity_mediana >= 40 & humidity_mediana <= 60 ~ 1,
        humidity_mediana >= 0 & humidity_mediana < 40 ~ 0.9,
        humidity_mediana > 60 & humidity_mediana <= 70 ~ 0.7,
        TRUE ~ -1
      ),
      total_score = 0.2 * temp_mediana_score +
        0.25 * wind_score +
        0.05 * clouds_mediana_score +
        0.45 * (0.6 * rain_score + 0.4 * snow_score) +
        0.05 * humidity_mediana_score
    )
}
...
calculate_photographic_score <- function(data) {
  data %>%
    mutate(
      ...
      total_score = 0.2 * temp_mediana_score +
        0.15 * wind_score +
        0.15 * clouds_mediana_score +
        0.45 * (0.5 * rain_score + 0.5 * snow_score) +
        0.05 * humidity_mediana_score
    )
}
  ...
  data %>%
    mutate(
      temp_mediana_score = case_when(
        ...
        temp_mediana > 25 & temp_mediana <= 30 ~ 0.7,
        TRUE ~ -1
      ),
      wind_score = case_when(
        wind_speed_max < 3.3 ~ 1,
        wind_speed_max >= 3.3 & wind_speed_max <= 5.4 ~ 0.5,
        wind_speed_max > 5.4 & wind_speed_max <= 7.9 ~ 0.1,
        wind_speed_max > 7.9 ~ -1
      ),
      clouds_mediana_score = case_when(
        clouds_mediana < 20 ~ 0.8,
        clouds_mediana >= 20 & clouds_mediana <= 50 ~ 1,
        TRUE ~ -0.2
      ),
      rain_score = ifelse(rain == "Yes", -1, 1),
  ...
  data %>%
    mutate(
      ...
      temp_score = case_when(
        ...
        temp_mediana >= -15 & temp_mediana < -10 ~ 0.7,
        temp_mediana >= -2 & temp_mediana <= 2 ~ 0.5,
        TRUE ~ -1.0
      ),
      clouds_mediana_score = case_when(
      ...
      humidity_mediana_score = case_when(
        ...
        humidity_mediana > 60 & humidity_mediana <= 70 ~ 0.5,
        TRUE ~ -0.2
      ),
      total_score = 0.6 * temp_score +
        0.1 * wind_score +
        0.05 * clouds_mediana_score +
        0.2 * (0.9 * rain_score + 0.1 * snow_score) +
        0.05 * humidity_mediana_score
    )
}
...
#Funckja do obliczania odległości (wzór Haversine)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      h4("Filtry wyszukiwania"),  # Nagłówek dla sekcji filtrów
      dateRangeInput(
        "selected_date",
        "Wybierz zakres dat:",
      ...
      # Wybór miasta wyjazdu
      selectInput(
        "departure_city",
        "Wybierz miasto wyjazdu:",
        choices = unique(forecast_summary$city),  # Lista miast z danych pogodowych
        selected = "Warsaw"  # Domyślnie wybrane miasto
      ),
      numericInput(
        "num_adults",
        "Liczba dorosłych (13+):",
      ...
      numericInput(
        "num_infants",
        "Liczba niemowląt (<2 lat):",
        value = 0,
        min = 0,
        max = 5
      ),
      actionButton("find_weather", "Znajdź najlepsze miejsce", class = "btn-primary"),
      br(), br(),
      p("Wskazówka: Wybierz daty i typ wyjazdu, aby znaleźć idealną lokalizację!")
    ),
    mainPanel(
      tabsetPanel(
        type = "tabs",
        tabPanel("Mapa najlepszych miejsc",
                 tags$div(
                   leafletOutput("map", height = "600px"),  # Dynamiczna mapa
                   style = "width: 100%; height: 100%;"
                 )),
        tabPanel("Tabela wyników",
                 tags$div(
                   DTOutput("score_table"),  # Dynamiczna tabela
                   style = "width: 100%;"
                 )),
      )
    )
...
server <- function(input, output, session) {
  observeEvent(input$find_weather, {
    ...
    # Wykluczenie miasta wyjazdu i miast w promieniu 40 km
    if (!is.null(input$departure_city)) {
      departure_city_coords <- forecast_summary %>%
        filter(city == input$departure_city) %>%
        summarise(lat = mean(lat), lon = mean(lon))  # Koordynaty miasta wyjazdu
      
      if (nrow(departure_city_coords) > 0) {
        filtered_data <- filtered_data %>%
          filter(
            city != input$departure_city,
            calculate_distance(
              lat, lon,
              departure_city_coords$lat, departure_city_coords$lon
            ) > 40  # 40 km
          )
      }
    }
    ...
    if (nrow(filtered_data) == 0) {
      ...
      output$airbnb_link <- renderUI(NULL)
    } else {
      ...
      avg_scores <- scored_data %>%
        group_by(city) %>%
        summarise(
          mean_score = round(mean(total_score, na.rm = TRUE) * 100,1),  # Wynik w %
          lat = mean(lat),  # Średnia szerokość geograficzna
          lon = mean(lon)   # Średnia długość geograficzna
        ) %>%
        arrange(desc(mean_score))
      
      # Dodanie klasteryzacji K-means
      num_clusters <- 10  # Liczba klastrów, dostosuj do swoich potrzeb
      kmeans_result <- kmeans(avg_scores$mean_score, centers = num_clusters)
      
      # Dodanie informacji o klastrze do wyników
      avg_scores <- avg_scores %>%
        mutate(cluster = kmeans_result$cluster)
      
      best_cluster <- avg_scores %>%
        group_by(cluster) %>%
        summarise(cluster_mean_score = mean(mean_score)) %>%
        arrange(desc(cluster_mean_score)) %>%
        slice(1) %>%
        pull(cluster)  # Pobierz numer najlepszego klastra
      
      filtered_avg_scores <- avg_scores %>%
        filter(cluster == best_cluster) %>%
        arrange(desc(mean_score))  # Posortuj malejąco po wynikach
      
      
      # Dodanie odległości od miasta wyjazdu
      if (!is.null(input$departure_city) && nrow(departure_city_coords) > 0) {
        avg_scores <- avg_scores %>%
          mutate(
            distance_km = round(calculate_distance(
              lat, lon,
              departure_city_coords$lat, departure_city_coords$lon
            ) / 10) * 10  # Zaokrąglenie do najbliższych 10 km
          )
      }
      
      # Wybranie 10 najlepszych lokalizacji
      top_matches <- scored_data %>%
        filter(city %in% avg_scores$city[1:10])  # Wybierz pierwsze 10 miast z najwyższą punktacją
      
      # Wyświetlenie najlepszych miejsc na mapie
      output$map <- renderLeaflet({
        map_data <- scored_data %>%
          filter(city %in% filtered_avg_scores$city)
      ...
      # Generowanie linku do Airbnb
      # Wyświetlenie tabeli z rankingiem z linkami do Airbnb
     output$score_table <- renderDT({
       filtered_avg_scores %>%
         mutate(
           Lp = row_number(),
           Airbnb_Link = paste0(
             "<a href='https://www.airbnb.com/s/",
             URLencode(city),
             "--Poland/homes?checkin=",
             format(input$selected_date[1], "%Y-%m-%d"),
             "&checkout=",
             format(input$selected_date[2], "%Y-%m-%d"),
             "&adults=",
             input$num_adults,
             "&children=",
             input$num_children,
             "&infants=",
             input$num_infants,
             "' target='_blank'>Zobacz noclegi</a>"
           )
         ) %>%
         select(Lp, Miasto = city, Dopasowanie = mean_score, Nocleg = Airbnb_Link) %>%
         datatable(
           escape = FALSE,
